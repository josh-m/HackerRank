...
def main():
    ...
    if (len(tokens) != 2):
        raise ValueError('Invalid initial input for N,T')
    
    N,T = [int(i) for i in tokens]
    
    #Get width array
    w_arr = raw_input().split()
    w_arr = [int(i) for i in w_arr]
    
    if (len(w_arr) != N):
        raise ValueError('Width array length:{0} does not match N:{1}'.format(len(w_arr),N))
        
    #evaluate the test cases
    for t in range(0,T):
        lanes = raw_input().split()
        if (len(lanes) != 2):
            raise ValueError('Invalid values for i,j')
        lanes = [int(i) for i in lanes]
        enter_pos, exit_pos = lanes 
    
        largest = min(w_arr[enter_pos:exit_pos+1])
